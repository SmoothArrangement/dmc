...
function theme_get_dropdown_menu($args = '') {
    ...
    foreach ($args['items'] as $item) {
        ...
        $items[] = new theme_MenuItem(
            array(
                'id' => $id,
                'attr' => array('class' => '', 'href' => $href, 'item_id' => $item_id, 'no_menu_trim_title' => $no_menu_trim_title),
                'title' => $title,
                'parent' => $parent,
...
function theme_get_list_categories($args = array()) {
	...
	foreach ($categories as $category) {
		...
		$items[] = new theme_MenuItem(array(
					'id' => $id,
					'active' => $active,
					'attr' => array('class' => ($active ? 'active' : ''), 'href' => get_term_link( (int) $id, $taxonomy )),
					'title' => $title,
...
class theme_MenuItem {
    ...
    function get_start($level) {
		...
		$class = '';
		$this->attr['class'] = ($this->active ? 'active' : '') . ' ';
        $link_class = implode(' ', $this->classes) . ' ' . $this->attr['class'];
        ...
        $output = call_user_func_array($item_start_function, array('class' => $class, 'title' => $title, 'attrs' => theme_prepare_attr($this->attr), 'link_class' => $link_class));
