...
<form action="<?php echo esc_url( $woocommerce->cart->get_cart_url() ); ?>" method="post" class=" bd-shoppingcarttable-1">
    ...
    <table class=" bd-table">
        ...
        <tfoot>
            <tr>
                <td colspan="6">
                    <div class=" bd-container-4 bd-tagstyles bd-custom-button bd-custom-bootstrapinput">
                        <input class=" bd-button-22" type="submit" name="update_cart" value="<?php _e('Update Cart', 'woocommerce'); ?>" />
                        <input class=" bd-button-22" type="submit" name="proceed" value="<?php _e('Proceed to Checkout &rarr;', 'woocommerce'); ?>" />
                        <?php
                            ...
                            if ( get_option( 'woocommerce_enable_coupons' ) == 'yes' && get_option( 'woocommerce_enable_coupon_form_on_cart' ) == 'yes') { ?>
                                <div class="form-inline form-responsive-dependent-float">
                                    ...
                                    <input class=" bd-button-22" type="submit" name="apply_coupon" value="<?php _e('Apply Coupon', 'woocommerce'); ?>"/>
                                </div>
                    ...
                    if ( $_product->exists() && $values['quantity'] > 0 ) {
                        ...
                        <tr <?php if ($i % 2 === 0): ?>class="alt"<?php endif ?>>
                            ...
                            <td class="product-thumbnail">
                                <?php $thumbnail = apply_filters( 'woocommerce_in_cart_product_thumbnail', $_product->get_image('shop_thumbnail', array('class' => ' bd-imagestyles-8')), $values, $cart_item_key ); ?>
    ...
    ?>
                            ...
                            <td class="product-quantity">
                                ...
                                if ( $_product->is_sold_individually() ) {
                                    ...
                                    $product_quantity = sprintf( '<div class="quantity"><input type="text" name="cart[%s][qty]" data-min="%s" data-max="%s" value="%s" size="4" title="Qty" class="qty  bd-bootstrapinput-6 form-control input-sm" maxlength="12" /></div>', $cart_item_key, $data_min, $data_max, esc_attr( $values['quantity'] ));
                                }
